...
summary: Veja as recomendações de atualização do firmware de storage, firmware do SP/BMC, arquivos de sistema e arquivos de segurança. 
---
...
.Passos
. Na página do painel de controle, selecione *Outras recomendações* no widget *Upgrade Advisor*.
. Selecione *Ver recomendações* para visualizar a lista de recomendações.
. Na coluna *Status da atualização*, selecione *Ver progresso* para visualizar os clusters que são elegíveis para uma atualização.
. Em caso de problemas, selecione os links correspondentes para visualizar as etapas para mitigar os problemas.
