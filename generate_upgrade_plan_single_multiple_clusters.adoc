...
---
= Gere um plano de atualização do ONTAP para clusters únicos e múltiplos no Digital Advisor.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Antes de gerar um plano de atualização, você deve:

* Certifique-se de que AutoSupport (ASUP) esteja habilitado e configurado corretamente, e que todas as seções necessárias estejam incluídas. Para obter mais informações, consulte link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide["Guia de Resolução de Problemas do AutoSupport do ONTAP"].
* Verifique se os membros do cluster do AutoSupport correspondem à configuração do cluster SAP.  Para obter mais informações,link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case["abrir um caso não técnico"] .
...
.Cluster único
--
. No painel de controle, selecione o número de clusters no widget *Upgrade Advisor*.
+
...
. Selecione um cluster para atualização. A coluna *Versão do SO de destino* exibe a versão recomendada do SO de destino. Você pode selecionar o ícone image:edit_icon.png["Imagem do botão de edição"] para escolher outra versão do SO de destino para um cluster.
+
image:r_ua_select_target_OS_version_single_cluster.png["Uma imagem que mostra como selecionar a versão de destino."]

+
** Você pode selecionar o número de nós na coluna *Nodes* para visualizar o resumo dos nós de um cluster.
** Você pode selecionar *Resolve Issues* na coluna *Recommended Action* para corrigir os problemas com um cluster inelegível para torná-lo elegível para uma atualização. + image:r_ua_resolve_issue.png["Uma imagem que mostra o tipo de problema e a resolução necessária para corrigi-lo."]


. Selecione *Gerar Plano de Atualização*. Você será redirecionado para a página *Recomendação de Atualização*.
...
** Você pode selecionar o ícone image:ua_export_icon.png["botão export (exportar)"] para exportar o resumo de risco para uma planilha do Excel.


. Selecione *Gerar Plano de Atualização* na página *Recomendação de Atualização*.
...
. Selecione *Gerar*. Você será redirecionado para a página *Relatórios*.
. Você pode baixar o plano de atualização da página *relatórios* quando estiver disponível.
+

TIP: Você pode selecionar *Visualizar Relatórios do Assistente de Atualização* para acessar a página *Relatórios*. + image:r_ua_view_reports.png["Uma captura de tela que mostra a posição onde o View Upgrade Advisor está localizado para seleção."]
...
. Selecione os clusters para atualização. A coluna *Versão do SO de destino* exibe a versão recomendada do SO de destino. Você pode selecionar o ícone image:edit_icon.png["Imagem do botão de edição"] para escolher outra versão do SO de destino de um cluster.
+
image:r_ua_select_target_OS_version.png["Uma imagem que mostra como selecionar a versão de destino."]

+
** Você pode selecionar o número de nós na coluna *Nodes* para visualizar o resumo dos nós de um cluster.
** Você pode selecionar *Resolve Issues* na coluna *Recommended Action* para corrigir os problemas com clusters inelegíveis para torná-los elegíveis para um upgrade. + image:r_ua_resolve_issue.png["Uma imagem que mostra o tipo de problema e a resolução necessária para corrigi-lo."]


. Selecione *Gerar Plano de Atualização*.
